...
function setup() {
    ...
    //rewards
    wand = createSprite(wizard.x+16,wizard.y+4);
    wand.addImage('StoneWandImage', stoneWandImage);
    wand.addImage('EmeraldWandImage', emeraldWandImage);
    wand.addImage('RubyWandImage', rubyWandImage);
    wand.scale = 1.65;
    wand.changeImage('StoneWandImage');
...
function draw() {
    ...
    //  spawnTiles(230,randomImg)
     ...
     stroke("teal");
     strokeWeight(2);
     text("Press Right Arrow Key to Start", 345,150);
    }
    //--------------------------------------------PLAY GAME STATE---------------------\\
    if(gameState === "play"){
     zombie.visible = true;
    ...
    //  }
     ...
     //stoneWand.x = wizard.x + 21;
     if(keyDown(RIGHT_ARROW)) {
        wizard.x = wizard.x+5;
     }
     if(keyDown(UP_ARROW) && wizard.y>85 ) {
        wizard.y = wizard.y-5; 
     }
     if(keyDown(DOWN_ARROW) && wizard.y<205 ) {
        wizard.y = wizard.y+5;
     }
     if(keyDown(LEFT_ARROW)) {
        wizard.x = wizard.x-5;
     }
     if(wizard.isTouching(chest)) {
        gameState = "stage2";
        stageOver();
     }
    ...
    swal(
      {
        ...
        text: "You Have Finished This Stage!",
